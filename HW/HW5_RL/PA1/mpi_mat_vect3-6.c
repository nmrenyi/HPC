...
void Mat_vect_mult(double parallel_result[], double local_A[], double local_x[], 
      ...
      int comm_sz, MPI_Comm comm, double* parallel_calc_time, double* distribution_time, int, int);
void Serial_mat_vect_mult(double*, double* global_A, double* global_x, int n);
double calcDifference(double* parallel_result, double* serial_result, int n, double* calc_time, int thread_count);
/*-------------------------------------------------------------------*/
...
#  endif
   ...
   Mat_vect_mult(parallel_result, local_A, local_x, local_y, local_m, m, n, local_n, comm_sz, comm, &parallel_calc_time, &distribution_time, my_rank, thread_count);   
   if (my_rank == 0) {
   ...
   if (my_rank == 0) {
      ...
      double calc_L2_norm_time = 0.0;
      double L2_norm = calcDifference(parallel_result, serial_result, n, &calc_L2_norm_time, thread_count);
      printf("serial calculation complete, time cost = %f s\n", serial_calc_time);
      printf("\n\n");
      printf("==================Calculation Complete================\n");
      printf("========================REPORT========================\n");
      printf("Ex3.6 with omp\n");
      printf("Process number = %d, matrix order = %d, thread count = %d\n", comm_sz, n, thread_count);
      printf("L2 norm = %f, L2 norm calc time = %f\n", L2_norm, calc_L2_norm_time);
...
#  ifdef DEBUG
   ...
   *calc_time = omp_get_wtime() - calc_start;
   result = sqrt(result);
   return result;
}
...
void Mat_vect_mult(
   ...
   MPI_Barrier(comm);
   double local_calc_start = omp_get_wtime();
   Omp_mat_vect(local_A, local_x, my_y, local_m, local_n, thread_count);
   // for (i = 0; i < local_m ; i++) {
   //    my_y[i] = 0.0;
   ...
   double local_calc_time = omp_get_wtime() - local_calc_start;
