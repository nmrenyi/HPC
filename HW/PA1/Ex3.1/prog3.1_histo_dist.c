...
int Which_bin(float data, float bin_maxes[], int bin_count, 
      float min_meas);
int Print_histo(float bin_maxes[], int bin_counts[], int bin_count, 
      float min_meas);
...
void Find_bins(
      ...
      // using reduce to effiently sum the local bin counts into global bin counts
...
int Which_bin(float data, float bin_maxes[], int bin_count, 
      float min_meas) {
/* TODO */
      // bin_maxes[0] - min_meas is the bin width, since every bin has the same width
      // we can calculate the bin number easily
      return (int)((data - min_meas) / (bin_maxes[0] - min_meas));
}  /* Which_bin */
